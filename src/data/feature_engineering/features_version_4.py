...
def _get_types_list(row, prefix): 
    ...
    return [t.replace(prefix, '').lower() for t in config.POKEMON_TYPES if f"{prefix}{t}" in row and row[f"{prefix}{t}"] == 1]



# HELPER 1: K.O. and HP % (Unchanged)
...
# HELPER 3: DYNAMIC MATCHUP (Unchanged)
...
# MAIN ORCHESTRATOR FUNCTION: feature_engineering_version_4
...
def feature_engineering_version_4(
    ...
    """
    Orchestrator function (v4):
    Creates the final feature set, removing zero-importance features (boosts, turn_1_switch)
    and refining status features.
    """

    print("--- Running Feature Engineering (v4) ---")
    ...
    # --- 2. Turn 1 Aggregation (REMOVED) ---
    # The features 'p1_switched_turn_1' and 'p2_statused_turn_1' had
    # very low or zero importance, so we are dropping them.
    
    # --- 3. Integrate Dynamic Features ---
    ...
    # --- Utility Boost features (REMOVED) ---
    # This entire block ('utility_boost_..._adv') had 0.0 importance.
    
    # --- REFINED: Status Pressure ---
    print("Creating refined Status Pressure features (v4)...")
    ...
    # --- 6. NEW: Static Interaction Feature ---
    ...
    # --- 7. Final Cleanup (Updated List) ---
    print("Finalizing feature set and cleaning up...")
    
    # This list now contains only the component features we want to drop
    # Many were already removed from the functions above
    ...
    final_df = final_df.infer_objects(copy=False) # Fix for future warning

    print(f"Feature Engineering (v4) complete. Final feature count: {final_df.shape[1]}")
    return final_df