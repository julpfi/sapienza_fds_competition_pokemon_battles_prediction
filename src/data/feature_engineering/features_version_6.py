...
def _get_types_list(row, prefix): 
    # Helper to get a list of types from hot-encoded columns
...
def _create_timeline_features(turns_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Now includes both team average HP (relative) and team sum HP (total resource),
    as well as KO counts and KO advantage.
    """
    # ko counts
    ...
    # Fill na: 0.5 for AVG (neutral), 0.0 for SUM (no HP resource)
    ...
    # Final merge of ko features and hp features
    timeline_features_df = pd.merge(ko_df, hp_df, on='battle_id', how='outer')
    
    return timeline_features_df


# 2. status pressure (Refined from version 4)

def _create_status_pressure_features_v4(turns_df: pd.DataFrame) -> pd.DataFrame:
    # Calculates total turns spent afflicted by status and differentiating into two severity levels 
...
def _create_dynamic_matchup_features(turns_df: pd.DataFrame, teams_df: pd.DataFrame) -> pd.DataFrame:
    # Calculates p1s stab count and p2s attack effectiveness against p1s active Pokemon
    ...
    p1_types_map = p1_types_df.groupby('battle_id').apply(lambda x: pd.Series(x['p1_types'].values, index=x['name'])
                                                          .to_dict(), include_groups=False).to_dict()
    ...
    temp_df['p1_stab_flag'] = temp_df.apply(lambda row: 1 
                                            if (row['p1_move_type_clean'] in row['p1_current_types']) and (row['p1_move_details_category'] not in ['status', 'MISSING_MOVE']) 
                                            else 0, axis=1)
    
    temp_df['p2_eff_score'] = temp_df.apply(lambda row: _get_type_multiplier(row['p2_move_type_clean'], row['p1_current_types']) 
...
def feature_engineering_version_6(
    train: bool,
