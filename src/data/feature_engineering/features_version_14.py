...
def _create_timeline_features(
    turns_df: pd.DataFrame,
    teams_df: pd.DataFrame,
    pokemon_stats_map: dict,      # <-- ADDED ARGUMENT (from v14 logic)
    default_pokemon_stats: dict   # <-- ADDED ARGUMENT (from v14 logic)
) -> pd.DataFrame:
    ...
    # --- START OF ADDED BLOCK (v14 logic for Weighted HP) ---
    
    # 4. Calculate Weighted Average using base_hp
    
    # Base data for weighting (base_hp)
    ...
    # Calculate Weighted Avg P1: sum(pct * weight) / sum(weight)
    ...
    # Merge the new weighted features (ADDED)
    hp_df = pd.merge(hp_df, p1_team_weighted_avg_hp, on='battle_id', how='outer')
    hp_df = pd.merge(hp_df, p2_team_weighted_avg_hp, on='battle_id', how='outer')
    ...
    # FillNa for new weighted features (ADDED)
    ...
    # Advantage for new weighted features (ADDED)
    hp_df['team_hp_weighted_advantage'] = hp_df['p1_team_weighted_avg_hp'] - hp_df['p2_team_weighted_avg_hp']
...
# 4 Dynamic Matchup
# 4. Dynamic Matchup Features (Ported from v14 - SYMMETRIC)
def _create_dynamic_matchup_features(
    ...
    # --- P2 Type Logic (This is the new part) ---
    ...
    # P1 Effectiveness vs P2 (Symmetric part)
    ...
    # P2 Effectiveness vs P1 (Original part)
    ...
    agg_dict = {
        'p1_stab_flag': 'sum',
        'p2_stab_flag': 'sum', # NEW
        'p1_eff_score': [('p1_hits_p2_super_effective', lambda x: (x > 1).sum()), ('p1_hits_p2_not_effective', lambda x: (x < 1).sum())], # NEW
        'p2_eff_score': [('p2_hits_p1_super_effective', lambda x: (x > 1).sum()), ('p2_hits_p1_not_effective', lambda x: (x < 1).sum())]
    }
...
# 5. Lead Differentials Features (new version of _create_lead_differential_features)
...
# 17. Realized Damage Features (new feature from version 14 with pokedex)
...
def feature_engineering_version_14(
    ...
    timeline_hp_features = _create_timeline_features(turns_df=turns_df, teams_df=teams_df, pokemon_stats_map=pokemon_stats_map, default_pokemon_stats=default_pokemon_stats)
    ...
    timeline_features = _create_timeline_features_hp(turns_df=turns_df, teams_df=teams_df, battles_df=battles_df)
    ...
    # 5. Deployed Mean Stat Features (new version of _create_lead_differential_features)
