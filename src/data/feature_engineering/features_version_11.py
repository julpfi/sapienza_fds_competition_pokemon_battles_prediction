...
def _create_big_three_features(teams_df: pd.DataFrame, turns_df: pd.DataFrame) -> pd.DataFrame:
    ...
    p2_revealed_mons = turns_df.groupby('battle_id')['p2_pokemon_state_name'].unique().apply(set)
...
def _create_trading_features(turns_df: pd.DataFrame) -> pd.DataFrame:
    ...
    final_df = trade_df[['trade_turn_adv']]
    return final_df
...
def feature_engineering_version_11(
    train: bool,
    ...
    print(f"Feature engineering version 11 completed. Feature count: {final_df.shape[1]}")
    return final_df